...
def main():
    ...
    updater = CGANUpdater(models=(conv, dis),
                         iterator=train_iter,
                         optimizer={'conv': opt_conv, 'dis': opt_dis},
                         device=args.gpu)
    trainer = training.Trainer(updater, (args.epoch, 'epoch'), out=args.outdir)

    trainer.extend(extensions.snapshot(), trigger=(50, 'epoch'))
    trainer.extend(extensions.snapshot_object(conv, 'model_{.updater.epoch}'), trigger=(50,'epoch'))
    trainer.extend(extensions.snapshot_object(dis, 'model_{.updater.epoch}'), trigger=(50,'epoch'))
    trainer.extend(extensions.LogReport())
    trainer.extend(extensions.PrintReport(['epoch', 'conv/loss_conv_MSE', 'conv/loss_conv2dis','dis/loss_real', 'dis/loss_conv', 'dis/loss']))
